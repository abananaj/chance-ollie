...
function roots_gallery($attr)
{
    ...
    foreach ($attachments as $id => $attachment) {
        $permalink = ($link == 'attachment');

        $image = wp_get_attachment_link($id, $size, $permalink, false);

        if (1 == 1) {
            $image_src = wp_get_attachment_image_src($id, 'attachment');
            $image_src = @$image_src[0];
            $attributes = 'data-toggle="lightbox" data-gallery="'.$unique.'" ';
            $attributes.= 'data-title="'.htmlspecialchars($attachment->post_title).'" ';
            $attributes.= 'data-footer="'.htmlspecialchars($attachment->post_excerpt).'" ';
            $attributes.= 'data-parent=".gallery" data-remote="'.$image_src.'">';

            $image = preg_replace('/\>/', $attributes, $image, 1);
        }
        ...
        if (trim($attachment->post_excerpt)) {
            $output .= '<div class="caption sr-only">'.wptexturize($attachment->post_excerpt).'</div>';
        }
